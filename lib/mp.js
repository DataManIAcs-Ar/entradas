...
async function createPreference({ accessToken, order, event, venue, items, baseUrl }) {
  // Las entradas al precio de cara, y el cargo por servicio como
  // renglón APARTE. Dos motivos: MP tiene que cobrar el total (si
  // mandáramos solo el precio de cara, cobraría de menos), y el
  // comprador ve el desglose en la pantalla de Mercado Pago en vez de
  // descubrir un número más alto del que esperaba.
  ...
  if (order.service_fee_cents > 0) {
    mpItems.push({
      ...
      quantity:    1,
      unit_price:  order.service_fee_cents / 100,
      currency_id: 'ARS',
    });
  }

  // Control: la suma de los items TIENE que dar total_cents. Si no
  // coincide, el webhook rechaza el pago por monto distinto y nadie
  // entiende por qué.
  const suma = Math.round(mpItems.reduce((s, i) => s + i.unit_price * i.quantity, 0) * 100);
  if (suma !== Number(order.total_cents)) {
    throw new Error('items no suman el total: ' + suma + ' vs ' + order.total_cents);
  }
